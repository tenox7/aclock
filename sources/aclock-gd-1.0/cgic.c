...
static cgiParseResultType cgiParsePostMultipartInput() {
	...
	while (1) {
		...
		n->fileName = malloc(strlen(ffileName) + 1);
		if (!n->fileName) {
			goto outOfMemory;
		}
		strcpy(n->fileName, ffileName);
		n->contentType = malloc(strlen(fcontentType) + 1);
		if (!n->contentType) {
			goto outOfMemory;
		}
		strcpy(n->contentType, fcontentType);
		n->tfileName = malloc(strlen(tfileName) + 1);
		if (!n->tfileName) {
			goto outOfMemory;
		}
...
#define RAPPEND(string, ch) \
	{ \
		if ((string##Len + 1) == string##Space)  { \
			char *sold = string; \
			string##Space *= 2; \
			string = realloc(string, string##Space); \
			if (!string) { \
				string = sold; \
				goto outOfMemory; \
			} \
	...
	char *name, char *result, int resultSpace)
{
	...
	} else if (strlen(e->fileName) > (resultSpace - 1)) {
	...
	char *name, char *result, int resultSpace)
{
	...
	} else if (strlen(e->contentType) > (resultSpace - 1)) {
		return cgiFormTruncated;
	} else {
		return cgiFormSuccess;
	}
...
cgiEnvironmentResultType cgiReadEnvironment(char *filename) {
	...
	while (1) {
		...
		if (fileFlag) {
			...
			rewind(out);
			e->tfileName = malloc(strlen(tfileName) + 1);
			if (!e->tfileName) {
...
{
	return cgiHtmlEscapeData(s, strlen(s));
}
...
{
	return cgiValueEscapeData(s, strlen(s));
}
